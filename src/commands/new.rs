...
use crate::client::{get_problems, last_viewed, resolve_problem, session_for_reads, title_of};
use crate::config;
use crate::language;
use crate::opener;
use crate::templates;
...
pub fn run(
    ...
    // -t wins, then KAFAE_TEMPLATE, then the grader's table, then the file, then the
    // builtin every course can use
    let wanted = config::load().template(template);
    let start = templates::pick(&wanted.value).unwrap_or_else(|reason| fail(&reason));
    ...
    let probs: Vec<Value> = if problem.contains(['*', '?', '[']) {
        ...
        let mut hits: Vec<Value> = get_problems(&session)
            .into_iter()
        ...
        vec![resolve_problem(&session, problem)]
    };
    ...
    for prob in &probs {
        ...
        let (ext, bytes) = match templates::open(&start, &session, prob, name, &title) {
            Ok(opened) => opened,
            Err(reason) => {
